...
export default function PatientDashboard({ user }: Props) {
  ...
  const patient = patientId !== null ? patients.find((p) => p.id === patientId) : undefined;
  ...
  if (patientId === null) {
    return (
      <div className="px-4 py-10 text-center space-y-2">
        <p className="text-slate-600 text-sm font-medium">Não foi possível carregar seus dados.</p>
        <p className="text-slate-400 text-xs">Fale com a recepção para verificar seu cadastro.</p>
      </div>
    );
  }
