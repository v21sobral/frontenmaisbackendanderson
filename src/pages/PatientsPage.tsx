...
export default function PatientsPage({ user }: Props) {
  ...
  // Cadastro de paciente disponível para todos os perfis com acesso a esta
  // tela (Admin e Atendente), conforme o caso de uso original do projeto.
  const canCreate = true;

  const openAdd = () => {
    if (!canCreate) return;
    setForm(empty); setErrors({}); setSelected(null); setModal("add");
  };
  ...
  const handleSave = () => {
    ...
    if (modal === "add") {
      if (!canCreate) { setModal(null); return; }
      ...
      const provisionalPassword = normalizeCPF(form.cpf).slice(0, 6);
  ...
  return (
    <div className="px-4 py-5 md:px-8 md:py-6">
          ...
          <div className="space-y-3.5">
            ...
            <div>
              ...
              {modal === "add" && !errors.email && (
                <p className="text-xs text-slate-400 mt-0.5">Esse e-mail será usado como login do paciente.</p>
              )}
            </div>
            <div>
      ...
      {credentials && (
        <Modal title="Paciente cadastrado" onClose={() => setCredentials(null)}>
          <p className="text-sm text-slate-600 mb-4">
            O acesso ao sistema foi criado automaticamente. Repasse estes dados ao paciente — ele será obrigado a criar uma nova senha no primeiro login.
          </p>
