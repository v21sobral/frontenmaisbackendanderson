...
export default function AppointmentsPage({ user }: Props) {
  ...
  const canSchedule = !isPatient && !isDoctor; // admin, attendant (visão/edição/busca)
  const canCreateAppointment = isAttendant; // apenas atendente cria/agenda nova consulta
  ...
  const handleAdd = () => {
    if (!canCreateAppointment) { setModal(null); return; }
