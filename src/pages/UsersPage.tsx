...
interface SysUser { id: number; name: string; email: string; role: Role; active: boolean; password: string; }
...
export default function UsersPage({ user }: Props) {
  ...
  const { sysUsers: rawList, setSysUsers } = useData();
  const list = rawList as SysUser[];
  const setList = setSysUsers as React.Dispatch<React.SetStateAction<SysUser[]>>;
