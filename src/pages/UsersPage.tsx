...
export default function UsersPage({ user }: Props) {
  ...
  const openEdit = (u: SysUser) => { setSelected(u); setForm({ name: u.name, email: u.email, role: u.role as Role, active: u.active, password: "" }); setErrors({}); setModal("edit"); };
  ...
  const handleSave = () => {
    ...
    if (modal === "add") {
      ...
      const detail = [
        ...
        selected.role !== form.role ? `Perfil: "${ROLE_LABELS[selected.role as Role] ?? selected.role}" → "${ROLE_LABELS[form.role]}"` : "",
          ...
          <div key={u.id} className="bg-white border border-slate-100 rounded-2xl p-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col items-end gap-1">
                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${ROLE_COLORS[u.role as Role] ?? "bg-slate-50 text-slate-500 border-slate-200"}`}>
